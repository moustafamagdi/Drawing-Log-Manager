...
function injectStyles(){
  ...
  const s=document.createElement('style');s.id='authEnhancementStyles';s.textContent=`
    .auth-card{width:min(460px,100%)}.auth-tabs{display:grid;grid-template-columns:1fr 1fr;background:#f2f5f9;padding:4px;border-radius:10px;margin:22px 0 6px}.auth-tab{border:0;background:transparent;border-radius:8px;padding:9px;cursor:pointer;font-weight:700;color:#697589}.auth-tab.active{background:#fff;color:#172033;box-shadow:0 1px 5px rgba(20,35,60,.1)}.password-wrap{position:relative}.password-wrap input{padding-right:62px}.password-toggle{position:absolute;right:8px;top:50%;transform:translateY(-50%);border:0;background:transparent;color:#1769e0;font-size:12px;font-weight:750;cursor:pointer;padding:6px}.auth-switch{text-align:center;margin:3px 0 0}.auth-error{min-height:17px;color:#a52b35;font-size:12px}.field-hint{font-size:11px;color:#788396;font-weight:500}.auth-card .stack{margin-top:14px}.auth-card h1{margin-bottom:4px}.auth-row-between{display:flex;justify-content:flex-end;margin-top:-7px}.forgot-link{font-size:12px}.reset-card{width:min(440px,calc(100vw - 30px))}.reset-note{margin:0 0 12px}.reset-success{text-align:center;padding:8px 0}`;document.head.appendChild(s);
}
function injectUi(){
...
function openResetRequest(){$('passwordResetModal').classList.remove('hidden');$('requestResetView').classList.remove('hidden');$('newPasswordView').classList.add('hidden');$('resetSuccessView').classList.add('hidden');$('resetModalTitle').textContent='Forgot Password';$('resetRequestError').textContent='';$('resetEmail').value=$('email')?.value||'';setTimeout(()=>$('resetEmail').focus(),50)}
function openNewPassword(){$('authView')?.classList.remove('hidden');$('appView')?.classList.add('hidden');$('passwordResetModal').classList.remove('hidden');$('requestResetView').classList.add('hidden');$('newPasswordView').classList.remove('hidden');$('resetSuccessView').classList.add('hidden');$('resetModalTitle').textContent='Set New Password';$('newPasswordError').textContent=''}
function closeReset(){$('passwordResetModal').classList.add('hidden');if(window.location.hash.includes('type=recovery'))history.replaceState(null,'',siteUrl())}
