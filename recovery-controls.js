...
if(cfg){
  ...
  let isAdmin=false,observer=null;
  ...
  async function init(){
    ...
    const {data}=await supabase.rpc('current_user_role');isAdmin=data==='admin';if(!isAdmin)return;
    ensureButton();
    const details=$('detailsPage');
    if(details&&!observer){
      observer=new MutationObserver(()=>queueMicrotask(ensureButton));
      observer.observe(details,{childList:true,subtree:true});
    }
  }
  function ensureButton(){const actions=document.querySelector('#detailsPage .detail-actions');if(!actions||$('moveRecycleBtn'))return;const b=document.createElement('button');b.id='moveRecycleBtn';b.className='btn secondary admin-only';b.type='button';b.textContent='Move to Recycle Bin';b.onclick=moveToRecycle;actions.appendChild(b)}
